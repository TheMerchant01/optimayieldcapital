...
export default function EditDepositHistory({ id, amount, dateAdded, email }) {
  const [loading, isloading] = useState(false);
  const [newAmount, setNewAmount] = useState(amount || "");
  const [selectedDate, setSelectedDate] = useState(dateAdded || "");
  const [errors, setErrors] = useState({
    newAmount: "",
    selectedDate: "",
  });

  console.log(id ,amount, dateAdded)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!newAmount) {
      newErrors.newAmount = "Amount is required.";
      formIsValid = false;
    }
    ...
    try {
      const response = await axios.post("/db/editDepositHistory/api", {
        newAmount,
        email: email,
        selectedDate,
        id,
  ...
  return (
    <div className="px-4 mt-20">
      <div className="settings px-3 py-4 rounded-md shadow-md shadow-gray-100">
        <div className="profile font-bold">Edit Deposit History</div>
        <div className="email flex items-center gap-x-1 bg-gray-50 p-2 rounded-md mt-2">
          ...
          <div className="email_title text-sm font-bold">{id}</div>
        </div>
      </div>
